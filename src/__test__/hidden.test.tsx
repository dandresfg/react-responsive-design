...
describe('Hidden Component', () => {
	...
	it('Should render', () => {
		...
		it("Shouldn't render on '2xl' media query", () => {
			const hiddenComponent = render(
				<MediaProvider>
					<div role='parent'>
						<Hidden media='2xl'>
							<p>Hello From Hidden out of 2 Extra Large Query</p>
						</Hidden>
					</div>
				</MediaProvider>
			);
			...
			expect(
				hiddenComponent.getByText(
					'Hello From Hidden out of 2 Extra Large Query'
				),
				'Hidden should render on all breakpoints except 2 Extra Large'
			).toBeTruthy();

			console.log("This test isn't implemented since 2xl is still false");

			// //Resize event to 2 Extra large
			// window.resizeTo(1600, 500);
			// fireEvent(window, new Event('resize'));

			// expect(
			// 	parentComponent.getElementsByTagName('p'),
			// 	"Hidden shouldn't render on 2 Extra Large Breakpoints"
			// ).length(0);
		}),
